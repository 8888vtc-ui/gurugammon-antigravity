...
export const Game: React.FC = () => {
    ...
    const [gameResult, _setGameResult] = useState<'single' | 'gammon' | 'backgammon'>('single');
    ...
    // Subscribe to game updates when connected
    useEffect(() => {
        if (isConnected && id) {
            subscribeToGame(id);
        }
    }, [isConnected, id, subscribeToGame]);
    ...
    useEffect(() => {
        if (lastMessage) {
            if (lastMessage.type === 'gameUpdate' && lastMessage.gameId === id) {
                setGameState(lastMessage.payload as GameState);
                setRolling(false);
                setCoachAdvice(null);
            }
            if (lastMessage.type === 'chat' && lastMessage.gameId === id) {
                const chatPayload = lastMessage.payload as { sender: string, text: string };
    ...
    return (
        <div className="min-h-screen bg-[#0F0F0F] text-white flex flex-col font-sans">
            ...
            {/* Game End Modal */}
            <GameEndModal
                isOpen={isGameEndModalOpen || gameState?.status === 'completed'}
                onClose={() => setIsGameEndModalOpen(false)}
                winner={gameState?.winner || null}
                loser={gameState?.winner?.id === gameState?.player1?.id ? gameState?.player2 : gameState?.player1}
                isPlayerWinner={gameState?.winner?.id === user.id}
                currentPlayerColor={isPlayer1 ? 'white' : 'black'}
                gameResult={gameResult}
                pointsWon={1}
                cubeValue={gameState?.cube?.level || 1}
                matchScore={{
                    white: gameState?.whiteScore || 0,
                    black: gameState?.blackScore || 0
                }}
                matchLength={gameState?.matchLength || null}
                onRematch={async () => {
                    try {
                        const { data } = await api.post('/games', {
                            mode: gameState?.player2?.id === 'ai-gurubot' ? 'AI_VS_PLAYER' : 'PLAYER_VS_PLAYER',
                ...
                onShare={() => {
                    if (navigator.share) {
                        navigator.share({
                            title: 'GuruGammon Game',
                            text: `I just ${gameState?.winner?.id === user.id ? 'won' : 'played'} a game on GuruGammon!`,
